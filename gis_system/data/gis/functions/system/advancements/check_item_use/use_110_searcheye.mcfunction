...
execute store result score @s gis_instant1 if entity @e[distance=..10,tag=gis_spawner,type=minecraft:armor_stand]
...
tellraw @s [{"text":"周囲の"},{"score":{"name":"@s","objective":"gis_instant1"}},{"text":"個のスポナーを発光させた"}]
execute store success score @s gis_instant1 run item replace entity @s[nbt={SelectedItem:{tag:{ID:110}}}] weapon.mainhand with minecraft:air
execute if score @s gis_instant1 matches 0 run item replace entity @s weapon.offhand with minecraft:air 
advancement revoke @p only gis:system/check_item_use/use_110_searcheye
