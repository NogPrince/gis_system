...
execute as @a at @s if predicate gis:system/position_check/in_boss_n if block ~ 43 ~ minecraft:redstone_block run summon minecraft:magma_cube ~ ~ ~ {PortalCooldown:3,Size:0,wasOnGround:1b,ActiveEffects:[{Id:14,Amplifier:1b,Duration:-1,ShowParticles:0b}],Tags:["gis_mob","gis_kill"],CustomName:'{"text":"業火の魔導師の炎の滝"}',Attributes:[{Name:generic.attack_damage,Base:80}]}
