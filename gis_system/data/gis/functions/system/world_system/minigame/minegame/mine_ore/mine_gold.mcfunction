execute as @a[tag=minegame] at @s if predicate gis:system/position_check/in_minegame_area run scoreboard players operation @s gis_mine_score += #minegame gis_mine_gold
execute as @a[tag=minegame,nbt={ActiveEffects:[{Id:26b,Amplifier:-128b}]}] at @s if predicate gis:system/position_check/in_minegame_area run scoreboard players operation @s gis_mine_score += #minegame gis_mine_gold
